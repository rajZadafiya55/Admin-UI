...
export default function Table() {
  ...
  const handleDeleteClick = (row) => () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://localhost:5000/api/table/delete/${row.row._id}`).then((res) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/table/getAll').then((r) => {
  ...
  return (
    <>
      <Container className="mt-4">
        ...
        <Card
          style={{ height: 480, width: '45%', backgroundColor: '#ffffff' }}
